...
async function feishuGetRaw(path, params = {}) {
  ...
  if (resp.data.code !== 0) {
    const err = new Error(`Feishu ${path}: ${resp.data.msg}`);
    err.response = { status: resp.data.code >= 10000 ? 500 : 400 };
    throw err;
  }
...
async function feishuGet(path, params = {}) {
  return withRetry(() => feishuGetRaw(path, params), { retries: 3, delay: 1000, backoff: 2 });
}
...
// Fetch all wiki nodes with recursive parent-child hierarchy (max depth 5)
export async function fetchWikiNodes() {
  try {
    const allNodes = [];

    // Fetch root-level nodes (no parent)
    let pageToken = '';
    do {
    ...
    // Recursively fetch children for each root node
    await fetchChildrenRecursive(allNodes, 0, 5);
...
async function fetchChildrenRecursive(allNodes, depth, maxDepth) {
  if (depth >= maxDepth) return;

  // Get the nodes at this depth level that we haven't fetched children for
  const nodesToProcess = allNodes.filter(n => n._depth === undefined || n._depth === depth);

  for (const node of nodesToProcess) {
    if (!node.node_token) continue;
    if (node._childrenFetched) continue;

    try {
      let childPage = '';
      do {
        const cparams = { page_size: 50, parent_node_token: node.node_token };
        if (childPage) cparams.page_token = childPage;
        const childData = await feishuGet(`/wiki/v2/spaces/${WIKI_SPACE_ID}/nodes`, cparams);
        const children = (childData?.items || []).map(c => ({ ...c, _depth: depth + 1 }));
        allNodes.push(...children);
        childPage = childData?.page_token || '';
      } while (childPage);
      node._childrenFetched = true;
    } catch (err) {
      console.warn(`Failed children of "${node.title}" (depth ${depth}):`, err.message);
    }
  }

  // Recurse for the next depth level
  const nextLevelNodes = allNodes.filter(n => n._depth === depth + 1 && !n._childrenFetched);
  if (nextLevelNodes.length > 0) {
    await fetchChildrenRecursive(allNodes, depth + 1, maxDepth);
  }
}
...
export async function downloadImage(token) {
  ...
  const localPathJpg = path.join(IMAGE_DIR, `${token}.jpg`);
  ...
  return withRetry(async () => {
    ...
    return `/images/${token}.jpg`;
  }, { retries: 3, delay: 2000, backoff: 2 });
}
